...
pepcomposer_jobs_archive_dir = 	join(pepcomposer_jobs_dir,"archived_jobs")
pepcomposer_log = 				join(pepcomposer_jobs_archive_dir,"pepcomposer.log")
...
# checking  paths
if os.path.exists(pepcomposer_path) == False:
	path_check_ok = False
if os.path.exists(pepcomposer_jobs_dir) = False:
	jobs_path_check_ok + False
if ! os.path.exists(pepcomposer_jobs_archive_dir) = False:
	os.mkdir (pepcomposer_jobs_archive_dir,2775)

if path_check_ok:
	# do pepcomposer stuff
else:
	if path_check_ok == False:
		print "ERROR!!! Pepcomposer directory not found in ", pepcomposer_path, "!"
	if jobs_path_check_ok == False:
